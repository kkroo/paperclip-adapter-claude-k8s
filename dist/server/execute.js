...
async function listClaudeSessionJsonlFiles(root, sessionId) {
    const files = [];
    const stack = [root];
...
async function quarantineClaudeSessionArtifacts(input) {
    ...
    if (moved > 0) {
        await input.onLog("stdout", `[paperclip] Quarantined ${moved} Claude session artifact(s) for immutable thinking failure: ${sessionId}\n`);
    }
}
async function tailPodLogFile(filePath, opts) {
    ...
    while (Date.now() < deadline) {
        try {
            ...
            if (stopSignal.stopped)
                throw new Error("Stop signal received before log file appeared");
 ...
 * Exported for unit tests.
 */
export function shouldAbortForCancellation(runStatus) {
    if (!runStatus)
        return false;
 ...
 */
function isInitOnlyRun(model, stdout) {
 ...
 */
function appendPodCause(message, state) {
    if (!state)
        return message;
    const parts = [];
    if (state.reason)
    ...
    if (state.signal !== null)
        parts.push(`signal=${state.signal}`);
    if (state.exitCode === 137)
        parts.push("SIGKILL (commonly OOMKilled)");
    if (parts.length === 0)
        return message;
 ...
 * Exported for unit tests.
 */
export function buildPartialRunError(exitCode, model, stdout, podState = null) {
    if (exitCode === 0)
    ...
    if (isInitOnlyRun(model, stdout) && (exitCode ?? 0) !== 0) {
        ...
        return appendPodCause(`Claude exited immediately after init${modelHint} (exit code ${exitCode ?? -1}) — the model may be unsupported or the session may have been rejected before producing output`, podState);
    }
    const initOnlyOutput = stdout.trim() !== "" && model !== "";
    if (initOnlyOutput) {
        const modelHint = model ? ` (model: ${model})` : "";
        return appendPodCause(`Claude started but did not produce a result${modelHint} — check API credentials, model support, and adapter config`, podState);
    }
 ...
 * Exported for unit tests.
 */
export function classifyOrphan(job, expected) {
    ...
    if (!expected.taskId || !jobTaskId)
        return "block_task_unknown";
    // taskId mismatch
    if (expected.taskId !== jobTaskId)
 ...
 */
async function waitForPod(namespace, jobName, scheduleTimeoutMs, startTimeoutMs, onLog, kubeconfigPath) {
...
async function waitForJobCompletion(namespace, jobName, timeoutMs, kubeconfigPath, observer) {
...
async function lookupPodState(namespace, jobName, kubeconfigPath) {
    const coreApi = getCoreApi(kubeconfigPath);
    const podList = await coreApi.listNamespacedPod({
        namespace,
 ...
 */
async function getPodLookupWithRetry(namespace, jobName, kubeconfigPath, attempts = 4, delayMs = 500) {
    let last = { state: null, phase: null, podMissing: true };
    for (let i = 0; i < attempts; i++) {
        ...
        if (last.phase !== "Succeeded" && last.phase !== "Failed")
            return last;
        if (i < attempts - 1)
...
async function getPodTerminatedState(namespace, jobName, kubeconfigPath) {
 ...
 * Exported for unit tests.
 */
export function describeTruncationCause(state, lookup) {
    if (!state) {
 ...
 */
async function cleanupJob(namespace, jobName, onLog, kubeconfigPath, podLogPath) {
    try {
        const batchApi = getBatchApi(kubeconfigPath);
...
export async function execute(ctx) {
    ...
    let _releaseMutex = () => { };
    const _mutexSlot = new Promise((resolve) => { _releaseMutex = resolve; });
    ...
    try {
        ...
        if (instructionsFilePath) {
            try {
                ...
                await onLog("stderr", `[paperclip] Warning: could not read agent instructions file "${instructionsFilePath}": ${err instanceof Error ? err.message : String(err)}\n`);
            }
        }
    ...
    let jobGoneDetectionPath = null;
    let jobGoneAt = null;
    const jobObserver = {
        lastConditions: null,
        pollCount: 0,
    };
    let podRunningAt = null;
    const activeJobRef = {
        namespace,
        jobName,
    ...
    try {
        ...
        keepaliveTimer = setInterval(() => {
            ...
            void (async () => {
                if (keepaliveJobTerminal || cancelled)
                    return;
    ...
    if (parsed &&
        ...
        if (isClaudeImmutableThinkingBlockError(parsed)) {
            const parsedSessionId = asString(parsed.session_id, "").trim();
    ...
    if (!parsed) {
        if (jobDeletedExternally && exitCode === null) {
            ...
            const detailParts = [];
            if (jobGoneDetectionPath)
                detailParts.push(`detected_via=${jobGoneDetectionPath}`);
