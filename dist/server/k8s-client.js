    ...
    if (!kc) {
        ...
        if (kubeconfigPath) {
            ...
            if (!process.env.KUBERNETES_SERVICE_HOST) {
                throw new Error("claude_k8s: in-cluster auth unavailable — KUBERNETES_SERVICE_HOST is unset (not running in a Kubernetes pod) and no kubeconfig path was provided");
            }
            try {
                kc.loadFromCluster();
            }
                ...
                throw new Error(`claude_k8s: failed to load in-cluster kubeconfig — the pod's ServiceAccount token is not mounted (set Helm serviceAccount.automountToken=true and rbac.create=true). Underlying error: ${cause}`);
            }
        }
        kcCache.set(key, kc);
    }
