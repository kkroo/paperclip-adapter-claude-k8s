import { readPaperclipRuntimeSkillEntries, resolvePaperclipDesiredSkillNames, readInstalledSkillTargets, } from "@paperclipai/adapter-utils/server-utils";
import path from "node:path";
const SKILLS_HOME = "/paperclip/.claude/skills";
async function buildK8sSkillSnapshot(config) {
    ...
    for (const [name, installedEntry] of installed.entries()) {
        if (availableEntries.some((e) => e.runtimeName === name))
            continue;
        entries.push({
            key: name,
            runtimeName: name,
...
export async function listK8sSkills(ctx) {
    return buildK8sSkillSnapshot(ctx.config);
}
export async function syncK8sSkills(ctx, _desiredSkills) {
    return buildK8sSkillSnapshot(ctx.config);
}
